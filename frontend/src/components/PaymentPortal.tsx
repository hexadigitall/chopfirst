...
export default function PaymentPortal({
  ...
  const cardPreview = (
    <div className="bg-gradient-to-br from-emerald-600 to-teal-600 rounded-xl px-4 py-3 text-white shadow-lg relative overflow-hidden">
      ...
      <div className="text-base font-mono tracking-widest mb-2">{cardNumber}</div>
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="p-4 pb-0">
          {cardPreview}
          <h3 className="text-base font-bold text-stone-800 mt-3 mb-1">Confirm Payment</h3>
          <p className="text-xs text-stone-500 mb-3">{label}</p>
        </div>
        <form onSubmit={handleSubmit} className="px-4 pb-4 space-y-2.5">
          <div>
            <label className="text-[11px] font-medium text-stone-500 mb-0.5 block">Card Number</label>
            <input
              value={cardNumber}
              onChange={e => setCardNumber(e.target.value)}
              className="w-full px-3 py-2 border border-stone-300 rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-emerald-500"
              maxLength={19}
              required
            />
          </div>
          <div className="grid grid-cols-2 gap-2.5">
            <div>
              <label className="text-[11px] font-medium text-stone-500 mb-0.5 block">Expiry</label>
              <input
                value={expiry}
            ...
            <div>
              <label className="text-[11px] font-medium text-stone-500 mb-0.5 block">CVV</label>
              <input
                value={cvv}
          ...
          <button
            type="submit"
            disabled={processing}
            className="w-full py-2.5 bg-emerald-600 text-white rounded-xl font-bold text-base hover:bg-emerald-700 transition-colors disabled:bg-emerald-300"
          >
          ...
          <button
            type="button"
            onClick={onCancel}
            className="w-full py-1.5 text-xs text-stone-400 hover:text-stone-600 transition-colors"
          >
            Cancel
          </button>
        </form>
